...
var abstractObj={}, onceInit=true, typeObj={} //=>{site:object}
...
function updateSelectSites(select){
  var type = select.options[select.selectedIndex].getAttribute("id");
  console.log("TYPE SITE: ", typeObj[type])
...
async function abstractionSiteHasBeenChosen(select){
  ...
  // for(var i=0; i<functions.length; ++i){
    var endpoint = abstractObj[site].functions[0].endpoint
    var name = abstractObj[site].functions[0].name
    var object = abstractObj[site].functions[0].object
    var arry = abstractObj[site].functions[0].array
    var searchParam = abstractObj[site].functions[0].searchParam;
  ...
  for(o in objects){
    ...
    // console.log(objects[o]);
    document.getElementById('object').innerHTML += '\n';
    document.getElementById('object').innerHTML += '// Get '+o;
    document.getElementById('object').innerHTML += '\n';

    var oName= o[0].toLowerCase() + o.slice(1);
    //get
    var func = site+'.'+ o +'("ADD ID")';
    document.getElementById('object').innerHTML += 'var '+oName+' = await '+func; //+ '&nbsp; //return a specific ' + o + ' from '+ site;
    document.getElementById('object').innerHTML += '\n';

    var properties = objects[o].properties;
    // document.getElementById('object').innerHTML += '\n/*** ' +o+' properties to get ***/\n//';
    document.getElementById('object').innerHTML += '// returns a ' +o+' with the following properties: \n';
    //Getter properties
    for(var p=0; p<properties.length; ++p){
      if(properties[p].type){
        document.getElementById('object').innerHTML += '// await '+oName+'.'+properties[p].property +'\n';
      }else{
    ...
    if(objects[o].setters){
      ...
      for(var s=0; s< setters.length; ++s){
        let obj = properties.find(o => o.property === setters[s].field);
        document.getElementById('object').innerHTML += obj.property;
        if(s+1<setters.length){
          document.getElementById('object').innerHTML +=', '
        }else{
          document.getElementById('object').innerHTML +='\n'
    ...
    if(objects[o].methods){
      for(let m in methods){//for each method
        ...
        firebase.database().ref('/apis/'+mEndpoint).once('value').then(function(snapshot) {
          ...
          document.getElementById('object').innerHTML += 'await '+oName+'.' + methods[m].name + args  +'\n';
        })
    ...
    if(objects[o].add){
      ...
      await firebase.database().ref('/apis/'+aEndpoint).once('value').then(function(snapshot) {
        ...
        document.getElementById('object').innerHTML += '\n// Create a new '+o+' object\n';
        document.getElementById('object').innerHTML += 'await '+site+'.'+o+'.create'+args +'\n';
...
function abstractionSiteHasBeenChosen2(select){
  //empty codes
  document.getElementById('function').innerHTML=""
  document.getElementById('object').innerHTML=""
  ...
  for(var i=0; i<scrapirAPIs.length; ++i){
    if(scrapirAPIs[i].title.split(' ').join('') == tempMaxTitleGet){//if the API endpoint is the GET chosen one
      ...
      for(var f=0; f<scrapirAPIs[i].res.length; ++f){
        ...
        //if does not include ID add it
        //if feild in suggestedPropFields, add both field and property
        let obj = suggestedPropFields.find(o => o.field === scrapirAPIs[i].res[f]);
        if(obj){
          propChosen=true;
          fieldChosen=true;
          globalProperty=obj.property
          globalField= scrapirAPIs[i].res[f];

          fieldHasBeenSelected(type+'_'+globalField)
...
function saveSchema(){
    ...
    if(typeof(tables[t].id) != undefined && tables[t].id != null){
      ...
      for(let r in propertyRows){
        ...
        if(! (propertyRows[r].id.includes('_row') || propertyRows[r].id.includes('_getM') || propertyRows[r].id.includes('_searchM') || !propertyRows[r].id.includes('_') || propertyRows[r].id.includes('show_') || propertyRows[r].id.includes('method_') || propertyRows[r].id.includes('search_') || propertyRows[r].id.includes('crud_') )){
          ...
          if(index != -1){
            ...
            console.log("schemaProObj[index].callerType: ", schemaProObj[index].callerType)
            console.log("**************")
            ...
            tempPropAPI.id = "";//tempObj[callerType].id;
            ...
            arrOfTypeProp.push({
              type: schemaProObj[index].type,
              construct: tpa.push(tempPropAPI),
              callerType:key
            })
      ...
      if(allSearchMethods!=[]){
        ...
        for(let s in allSearchMethods){
          ...
          siteObj.functions.push(tempAct);
        }
      }

      console.log("tempAct: ", tempAct)

      console.log("siteObj[site].functions: ", siteObj.functions)
  ...
  // console.log("SITEXX: ", site);
  // console.log("SITEOBJXX: ", siteObj);
...
// function saveTypes(){
//   firebase.initializeApp(config);
//   firebase.database().ref('/types/').set("{'name':'type'}");
// }

// function saveTypes(){
//   //go over all the siteIsClicked, and save the obecjts in an array, then push the array to object with key
//   firebase.initializeApp(config);
//   let ob = {};
//   let obArray = {}
//   var arr = [];
...
//     snapshot.forEach(function(childSnapshot) {
//       // key, val() => site, [types]
//       var types = childSnapshot.val()
//       for(var i=0; i<types.length; ++i){
//         // console.log()
//         // if()//if the types is one of the keys in this object, push the site to it, otherwise, create a new one
//         if(Object.keys(ob).indexOf(types[i]) ==-1){
//           ob[types[i]]=[]
//           ob[types[i]].push(childSnapshot.key)
//         }else{
//         ob[types[i]].push(childSnapshot.key)
//         }
//       }
//         // ob[childSnapshot.key]=[]
//         // ob[childSnapshot.key] = Object.keys(childSnapshot.val().objects)
//       });
//     })
//     // obArray = ob
//     console.log(ob);
...
function fieldHasBeenSelected(select){
  ...
  if(select.tagName === 'SELECT'){
    ...
    globalField = select.split('_')[1];
    var property = select.split('_')[1];
    var child = select.split('_')[1];
    var parent = select.split('_')[0];
    var thisType = select.split('_')[0];
  }
